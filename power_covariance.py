...
# -------------------------------- COVARIANCE POWER -------------------------------- #
def cov_power(powertype="power", mainpath="", simset=DeusPurSet("all_256"), isimmin=1, isimmax=2, noutput=1, aexp=0.,
              growth_a=np.zeros(0), growth_dplus=np.zeros(0), nmodel=0, okprint=False, store=False):
    ...
    if os.path.isfile(fname):
        ...
        power_k, power_pmean, power_psigma = mean_power(powertype, mainpath, simset, isimmin, isimmax, noutput,
                                                        aexp, growth_a, growth_dplus, nmodel, okprint)
        ...
        if powertype == "linear":
            ...
            for isim in xrange(isimmin, isimmax):
                ...
                dummy, power_p = power_spectrum(powertype, mainpath, true_simset, true_isim, noutput,
                                                aexp, growth_a, growth_dplus, nmodel, okprint)
...
def cov_power_kcut(kmin, kmax, powertype, mainpath="", simset=DeusPurSet("all_256"), isimmin=1, isimmax=2, noutput=1,
                   aexp=0., growth_a=np.zeros(0), growth_dplus=np.zeros(0), nmodel=0, okprint=False, store=False):
    ...
    if os.path.isfile(fname):
        ...
        if okprint:
            print "Computing power spectrum covariance"
        power_k, power_pmean, power_psigma, power_pcov_nocut = cov_power(powertype, mainpath, simset, isimmin, isimmax,
                                                                         noutput, aexp, growth_a, growth_dplus, nmodel,
                                                                         okprint)
        idx = [(power_k > kmin) & (power_k < kmax)]
        power_k_kcut = power_k[idx]
        power_pmean_kcut = power_pmean[idx]
        ...
        if store:
            f = open(fname, "w")
            for i in xrange(0, power_k_kcut.size):
                for j in xrange(0, power_k_kcut.size):
                    f.write(str(fltformat % power_pcov[i, j])+" ")
                f.write("\n")
            f.close()
...
def corr_coeff(powertype="power", mainpath="", simset=DeusPurSet("all_256"), isimmin=1, isimmax=2, noutput=1,
    ...
    if os.path.isfile(fname):
        ...
        power_k, power_pmean, power_psigma, power_pcov = cov_power(powertype, mainpath, simset, isimmin, isimmax,
                                                                   noutput, aexp, growth_a, growth_dplus, nmodel,
                                                                   okprint)
    ...
    return power_corr_coeff
# ---------------------------------------------------------------------------- #


# ----------------------------- SIGNAL TO NOISE ------------------------------ #
def signoise(powertype="nyquist", mainpath="", simset=DeusPurSet("all_256"), noutput=1, nsimmax=1, aexp=0.,
             growth_a=np.zeros(0), growth_dplus=np.zeros(0), nmodel=0, unbiased=False, okprint=False, store=False):

    fname = output_file_name("sig_noise", powertype, simset, 1, nsimmax, noutput, nmodel)
    fltformat = "%-.12e"
    ...
    if os.path.isfile(fname):
        ...
        power_k, power_pmean, dummy, power_pcov = cov_power(powertype, mainpath, simset, 1, nsimmax, noutput,
                                                            aexp, growth_a, growth_dplus, nmodel, okprint)
...
def signoise_k(powertype="nyquist", mainpath="", simset=DeusPurSet("all_256"), noutput=1, nsimmax=1,
               aexp=0., growth_a=np.zeros(0), growth_dplus=np.zeros(0), kmax=0.4, unbiased=False, okprint=False):

    nmax = nsimmax-1
    
    power_k, power_pmean, power_psigma, power_pcov = cov_power_kcut(0., kmax, powertype, mainpath, simset, 1, nsimmax,
