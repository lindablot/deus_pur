...
# --------------------------- PRECISION MATRIX VARIANCE --------------------------- #
def inv_cov_variance_kmax(kmax = 1., powertype = "power", mainpath = "", noutput = 1 , aexp = 1., growth_a = np.zeros(0), growth_dplus = np.zeros(0), list_nr = [300,500,700,1000,3000,5000,6000], okprint=False):
    ...
    for nr in list_nr:
        ...
        for isub in xrange(0, nsub):
            isimmin = isub * nr + 1
            isimmax = (isub+1)*nr
            filename="tmp/"+str("%05d"%noutput)+"/inv_cov_"+powertype+"_"+str("%05d"%nr)+"_"+str("%05d"%isub)+"_"+str("%1.2f"%kmax)+".txt"
            if(os.path.isfile(filename)):
                cov_inv=np.loadtxt(filename, unpack=True)
                #cov_inv= float(nr-1)*cov_inv/float(nr-nbin-2)
            else:
                kcov,dummy,dummy,cov=cov_power(powertype, mainpath, simset, isimmin, isimmax+1, noutput, aexp, growth_a, growth_dplus)
                cov_kmax = cov[0:ikmax,0:ikmax]
                cov_inv=linalg.inv(cov_kmax)
                #cov_inv= float(nr-nbin-2)*cov_inv/float(nr-1)
                fltformat="%-.12e"
                f = open(filename, "w")
                for ik in xrange(0, nbin):
